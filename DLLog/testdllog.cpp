...
static void testAsciLog(){
    DEBUG_LOG("hello debug log :%s","BBBBBBBBBBBBBBBBBB");
...
static void testWriteLog(){
    ...
    {
        WARNING_LOG("write warning log %s","DDDDDDDDDDDDDDDDDDD");
        ERROR_LOG("write error log %s","CCCCCCCCCCCCCCCCCCCCCCC");
        FATAL_LOG("write fatal log %s","AAAAAAAAAAAAAAAAAAAAAAa");
    }
...
static void testAssert(){
    ...
    setIsOpenAssert(0);
    DEBUG_LOG("assert debug log","ssss");
...
static void _linetest(){
    printf("linetest");
}

static void testLogMain(){
    testAsciLog();
    testUtf8Log();
    testString();
    testAssert();
